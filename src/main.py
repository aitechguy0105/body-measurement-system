...
from src.util import (is_valid_keypoint_1, pair_dir, int_tuple, preprocess_image, get_angle)
from shapely.geometry import LineString, Point, MultiPoint
from shapely.ops import nearest_points
from src.silhouette import  SilhouetteExtractor
from src.pytorch_openpose import torch_openpose
import time
str_fit_type = ['Regular', 'Extremly Slim', 'VerySlim', 'Slim', 'Regular Tight', 'Regular Loose', 'Loose', 'Very Loose', 'Extremely Loose']
...
str_arm_type = ["Arm_Normal", "Arm_Slight_Backward", "Arm_Very_Backward",  "Arm_Very_Forward", "Arm Curved",  "Arm_Straight"]
...
str_belly_type = ["Belly Normal", "Belly-Tenesmic stomach high", "Bend forward of abdomen", "Belly Portly", "Normal Fat Around"]
...
def cross_section_points(contour_string, center, line_p0, line_p1, nearest = True):
    ...
    if isect_points.geom_type == 'MultiPoint':
        isect_points = np.array([(p.x, p.y) for p in isect_points.geoms])
        if nearest == True:
...
def estimate_front_neck_points(is_front, contour, keypoints):

    neck = keypoints[POSE_BODY_25_BODY_PART_IDXS['Neck']][:2]
    nose = []
    if is_front == False:
        nose = (keypoints[POSE_BODY_25_BODY_PART_IDXS['REar']][:2] + keypoints[POSE_BODY_25_BODY_PART_IDXS['LEar']][
                                                                     :2]) / 2
    else:
        nose = keypoints[POSE_BODY_25_BODY_PART_IDXS['Nose']][:2]
...
def estimate_front_shoulder_points(contour_string, keypoints):
    ...
    # this parameter could be studied for further use
    up_dir = np.array([0, -1.0]) * 2
    p0 = lshoulder
...
def estimate_landmark_side_segments(contour_s, keypoints_s):
    ...
    if ret:
        landmarks_s['Aux_Thigh_0'] = points
...
def estimate_landmark_front_segments(is_front, contour_f, keypoints_f,
    ...
    points = estimate_front_neck_points(True, contour_f, keypoints_f)
...
def estimate_landmark_segments(contour_f, keypoints_f, contour_s, keypoints_s):
    ...
    landmarks_f = estimate_landmark_front_segments(True, contour_f, keypoints_f, armscye_ratio_s, bust_ratio_s, under_bust_ratio_s, crotch_ratio_s)
...
def approximate_body_measurement_as_ellipse_perimeter(measure_f, measure_s):
    ...
    if 'Aux_Thigh_0' in measure_f and measure_f['Aux_Thigh_0'] != -1 and measure_s['Aux_Thigh_0'] != -1:
...
def load_pair_info(pair_text_file_path):
    ...
    with open(pair_text_file_path) as f:
        for line in f.readlines():
            img_name_f, img_name_b, img_name_s, height, weight = line.split()
            height = float(height)
            weight = float(weight)
            pairs.append((img_name_f, img_name_b, img_name_s, height, weight))
    return pairs

def get_fit(weight, height):
    BMI = weight / (height ** 2)

def get_arm_type(img, sil, keypoints_s, h_px_ratio, result):
    ...
    lshoulder = np.array(keypoints_s[0][POSE_BODY_25_BODY_PART_IDXS['LShoulder']][:2], int)
    lelbow = np.array(keypoints_s[0][POSE_BODY_25_BODY_PART_IDXS['LElbow']][:2], int)
    lwrist = np.array(keypoints_s[0][POSE_BODY_25_BODY_PART_IDXS['LWrist']][:2], int)
    neck = np.array(keypoints_s[0][POSE_BODY_25_BODY_PART_IDXS['Neck']][:2], int)
    mid_hip = np.array(keypoints_s[0][POSE_BODY_25_BODY_PART_IDXS['MidHip']][:2], int)
    ...
    lwrist[1] *= -1
    lshoulder[1] *= -1
    ang_lshoulder_lelbow_lwrist = math.fabs(get_angle(lelbow, lshoulder, lwrist))
    dir_shoulder_neck = (lshoulder - neck) * h_px_ratio
    str_dis = str(dir_shoulder_neck[0]).format('.3f')
    str_ang = f'{ang_lshoulder_lelbow_lwrist:.2f}'
    # Arm Curved
    if ang_lshoulder_lelbow_lwrist < 160:
        type = 4
    # Arm Very Backward
    elif dir_shoulder_neck[0] > 2.5 :
        type = 2
    # Arm Very Forward
    elif dir_shoulder_neck[0] < -0.6:
        type = 3
    # Arm Normal
    elif ang_lshoulder_lelbow_lwrist < 5:
        type = 5
    # Arm slight backward
    elif dir_shoulder_neck[0] > 1.5:
        type = 1
    # normal
    else:
        type = 0
...
cnt = 0
def get_back_style(img_s, sil_s, keypoints_s, contour_s, landmark_s, h_px_ratio, result):
    ...
    type = 0
    # cv.imshow("src_image", img)
    nose = np.array(keypoints_s[0][POSE_BODY_25_BODY_PART_IDXS['Nose']][:2], int)
    neck = np.array(keypoints_s[0][POSE_BODY_25_BODY_PART_IDXS['Neck']][:2], int)
    mid_hip = np.array(keypoints_s[0][POSE_BODY_25_BODY_PART_IDXS['MidHip']][:2], int)
    ...
    dis_armscye_waist = (armscye[1][0] - aux_waist_underbust_0[1][0]) * h_px_ratio
    ...
    if ang_nose_neck_midhip < 135:
        # style S
        if dis_armscye_waist > 5:
            type = 4
        # humpack + forwardhead
        elif dis_armscye_waist > 4.5:
            type = 3
        # forward head
        else:
            type = 1
    # forward Head
        ...
        # backhumpack
        if dis_armscye_waist > 4.5:
            type = 2
        # normal
        else:
            type = 0
    result['back'] = type
    cv.putText(img_s, f"{str_back_type[type]}:{str_ang}, {str_dis}",
...
def get_belly_type(img_f, sil_f, keypoints_f, contour_f, landmark_f,  img_s, sil_s, keypoints_s, contour_s, landmark_s, h_px_ratio, result):
    clr_neck2hip = (255, 0, 0)
    thickness = 2
    clr_arm = (0, 255, 0)
    font = cv.FONT_HERSHEY_SIMPLEX
    ...
    waist_f = landmark_f["Waist"]
    aux_waist_underbust_0_f = landmark_f["Aux_Waist_UnderBust_0"]
    bust_f = landmark_f["Bust"]
    armscye_f = landmark_f["Armscye"]
    dis_waist_auxunderbust_f = (linalg.norm(waist_f[0] - waist_f[1]) - linalg.norm(aux_waist_underbust_0_f[0] - aux_waist_underbust_0_f[1])) * h_px_ratio
    dis_waist_armscye_f = (linalg.norm(waist_f[0] - waist_f[1]) - linalg.norm(
        armscye_f[0] - armscye_f[1])) * h_px_ratio
    dis_waist_bust_f = (linalg.norm(waist_f[0] - waist_f[1]) - linalg.norm(
        bust_f[0] - bust_f[1])) * h_px_ratio
    str_id = f'{bust[0][0]:.2f}'
    # tenesmic belly
    if waist[0][0] < bust[0][0] - 5 and waist[0][0]< aux_waist_underbust_0[0][0] :
        type = 1
    # bend forwad of abdomen
    ...
    # belly portly
        ...
        type = 3
    # fat around
    elif dis_waist_auxunderbust_f > -1 and dis_waist_bust_f > -1 and dis_waist_armscye_f > -1:
        type = 4
    # normal
    else:
        type = 0
    ...
    if type == 4:
        cv.putText(img_f, f"{str_belly_type[type]}",
                   int_tuple(aux_waist_underbust_0_f[0] - 50), font,
                   font_scale, text_color, font_thickness, cv.LINE_AA)
        draw_slice_data(img_f, contour_f, landmark_f)
        # cv.imshow("Belly Type", img_f)
        # cv.waitKey(0)
    else:

        cv.putText(img_s, f"{str_belly_type[type]}",
    ...
    #
def get_body_type(img_s, sil_s, keypoints_s, contour_s, landmark_s, h_px_ratio, result):
    ...
    dis_neck_armscye = (neck[1][0] - armscye[1][0]) * h_px_ratio
    dis_collar_armscye = (collar[1][0] - armscye[1][0]) * h_px_ratio
    str_id = f'{dis_neck_armscye:.2f}'
    if dis_neck_armscye < -7 and dis_collar_armscye < -6:
        type = 3
    elif dis_neck_armscye <  -5 and dis_collar_armscye < -4:
        type = 1
    elif dis_neck_armscye > -1 and dis_collar_armscye > -0.6:
        type = 2
    else:
        type = 0

    result['body_type'] = type
    cv.putText(img_s, f"{str_body_basic_type[type]}",
               font_coordinate, font,
               font_scale, text_color, font_thickness, cv.LINE_AA)
    draw_slice_data(img_s, contour_s, landmark_s)

     # cv.imwrite(f"{str_body_basic_type[type]}_{str_id}.jpg", img_s)
    # cv.imshow("Body Type", img_s)
    # cv.waitKey(0)
def get_hip_waist_type(img_s, sil_s, keypoints_s, contour_s, landmark_s,h_px_ratio, result):
    ...
    dis_hip_crotch = (hip[1, 0] - crotch[1,0]) * h_px_ratio
    dis_hip_aux_crotch = (hip[1, 0] - aux_crotch_hip_0[1, 0]) * h_px_ratio
    str_id = f'{dis_hip_crotch:.2f}'
    # drop seat
    if dis_hip_aux_crotch < -1.0:
        type = 3
    # prominent seat
    elif dis_hip_crotch > 3.3:
        type = 2
    # flat seat
    elif dis_hip_crotch < 1.0:
        type = 1
    # normal hip
    else:
        type = 0
    ...
    waist = landmark_s["Waist"]
    armscye = landmark_s["Armscye"]
    dis_waist_armscye = (waist[1, 0] - armscye[1, 0]) * h_px_ratio
    dis_waist_hip = (waist[1, 0] - hip[1, 0]) * h_px_ratio
    # very concative waist
    if dis_waist_armscye < -4.0 and dis_waist_hip > -2.0:
        waist_type = 2
    # slightly concative waist
    elif dis_waist_armscye < -2.0:
        waist_type = 1
    # waist normal
    else:
        waist_type = 0

    result['waist'] = waist_type
...
def rearrange_in_x_order(landmark_s):
    ...
    return landmark_s
def get_shoulder_type(img_f, sil_f, keypoints_f, contour_f, landmark_f, img_s, sil_s, keypoints_s, contour_s, landmark_s, h_px_ratio, result):
    ...
    result['shoulder_blade_inner_type'] = shoulder_blade_inner_type
    ...
    result['r_shoulder_angle']  =  r_type
    l_type = get_slope_loc(l_slope)
    result['l_shoulder_angle'] = l_type
    ...
    dis_auxarmscye_armscye = (aux_armscye_s[1, 0] - armscye_s[1, 0]) * h_px_ratio
    if dis_auxarmscye_armscye > 1.2:
        shoulder_bulge_blade_type = 2
    elif dis_auxarmscye_armscye > 3:
        shoulder_bulge_blade_type = 0.2
    else:
        shoulder_bulge_blade_type = 0

    result['shoulder_buldge_blade'] = shoulder_bulge_blade_type
    cv.putText(img_f, f"{str_shouler_blade_inner_type[shoulder_blade_inner_type]}",
               int_tuple(shoulder_f[0] + [50, 50]), font,
               font_scale, text_color, font_thickness, cv.LINE_AA)
    ...
    # cv.waitKey(0)
def get_chest_type(img_f, sil_f, keypoints_f, contour_f, landmark_f, img_s, sil_s, keypoints_s, contour_s, landmark_s, h_px_ratio, result):
    ...
    dis_armscye_waist = (armscye_width - waist_width) * h_px_ratio
    dis_armscye_bust_s = (armscye_s[0, 0] - bust_s[0, 0]) * h_px_ratio

    if dis_armscye_waist > 4.0:
        chest_type = 2
    ...
    result['chest'] = chest_type
    # cv.putText(img, "Arm Curved", (100, 100), font, font_scale, text_color, font_thickness, cv.LINE_AA)
    draw_slice_data(img_f, contour_f, landmark_f)
    draw_slice_data(img_s, contour_s, landmark_s)
...
def get_fit_type(img_f, sil_f, keypoints_f, contour_f, landmark_f, img_s, sil_s, keypoints_s, contour_s, landmark_s, measurement, result):
    ...
    waist_f = landmark_f['Waist']
    aux_waist_under_bust_f = landmark_f['Aux_Waist_UnderBust_0']

    dis_body_circumference = measurement['Waist_Circumference']

    fit_type = 0
    gap = 10
    dis_min_regular = 185
    if dis_body_circumference < dis_min_regular - gap * 3:
        fit_type = 1
    elif dis_body_circumference < dis_min_regular - gap * 2:
        fit_type = 2
    elif dis_body_circumference < dis_min_regular - gap * 1:
        fit_type = 3
    elif dis_body_circumference < dis_min_regular - gap * 0:
        fit_type = 4
    elif dis_body_circumference < dis_min_regular - gap * -1:
        fit_type = 0
    elif dis_body_circumference < dis_min_regular - gap * -2:
        fit_type = 5
    elif dis_body_circumference < dis_min_regular - gap * -3:
        fit_type = 6
    elif dis_body_circumference < dis_min_regular - gap * -4:
        fit_type = 7
    elif dis_body_circumference < dis_min_regular - gap * 5:
        fit_type = 8

    result['fit'] = fit_type
    cv.putText(img_f, f"{str_fit_type[fit_type]}{dis_body_circumference:.2f}", int_tuple(aux_waist_under_bust_f[0] - [50, 0]), font, font_scale, text_color, font_thickness, cv.LINE_AA)
    # cv.imshow("Fit Type", img_f)
    #
    # cv.waitKey(0)

class body_type_measurement():
    def __init__(self):
        print('init body type measurement')
        self.body_estimation = None
        self.sil_extractor = None
        self.load_model()

    def load_model(self):
        self.body_estimation = torch_openpose.torch_openpose('body_25')
        self.sil_extractor = SilhouetteExtractor(use_mobile_model=False, use_gpu=False)


    def handle_one_user_images(self, img_f, img_b, img_s,  height):
        result = {}
        ...
        # get keypoints
        keypoints_f = self.body_estimation(img_f)
        keypoints_s = self.body_estimation(img_s)
        # keypoints_b = self.body_estimation(img_b)
        # filled_keypoints_f = [sublist[:1] if sublist else [(0, 0, 0, 0)] for sublist in keypoints_f]
        # filled_keypoints_f = np.array(filled_keypoints_f)
        # filled_keypoints_f = np.squeeze(filled_keypoints_f)
        #
        # filled_keypoints_s = [sublist[:1] if sublist else [(0, 0, 0, 0)] for sublist in keypoints_s]
        # filled_keypoints_s = np.array(filled_keypoints_s)
        # filled_keypoints_s = np.squeeze(filled_keypoints_s)
        #
        # filled_keypoints_b = [sublist[:1] if sublist else [(0, 0, 0, 0)]  for sublist in keypoints_b]
        # filled_keypoints_b = np.array(filled_keypoints_b)
        # filled_keypoints_b = np.squeeze(filled_keypoints_b)
        # keypoints_f = filled_keypoints_f[np.newaxis,:, :3]
        # keypoints_s = filled_keypoints_s[np.newaxis,:, :3]
        # keypoints_b = filled_keypoints_b[np.newaxis,:, :3]
        # in case only front and back
        keypoints_f = np.array(keypoints_f)
        keypoints_s = np.array(keypoints_s)
        # return
        img_debug = img_f.copy()
        # get silhouette
        sil_deeplab_f, sil_refined_f = self.sil_extractor.extract_silhouette(img_f, 0, keypoints_f, img_debug=img_debug)
        # sil_deeplab_b, sil_refined_b = self.sil_extractor.extract_silhouette(img_b, 1, keypoints_b, img_debug=None)
        sil_deeplab_s, sil_refined_s = self.sil_extractor.extract_silhouette(img_s, 2, keypoints_s, img_debug=None)

        # get landmarks ====  contour, segment landmarks ===== with silhouette and keypoints
        data, final_viz = calc_body_landmarks_util(img_f, img_s, sil_refined_f,  sil_refined_s, keypoints_f, keypoints_s,
                                                   height=height)
        ...
        h_points = ordered_landmark_s['Height']
        bottom_h_point = h_points[0] if h_points[0][1] > h_points[1][1] else h_points[1]
        h_px_ratio = height_pixel_ratio(height, ordered_landmark_s['Height'])

        measurement = data['measurement']
        real_segment_len_f = data['landmark_segment_dst_f']
        real_segment_len_s = data['landmark_segment_dst_s']

        get_fit_type(img_f, sil_refined_f, keypoints_f, data['contour_f'], ordered_landmark_f, img_s, sil_refined_s,
                       keypoints_s, data['contour_s'], ordered_landmark_s, measurement, result)
        get_hip_waist_type(img_s, sil_refined_s, keypoints_s,data['contour_s'], ordered_landmark_s,h_px_ratio, result)
        get_chest_type(img_f, sil_refined_f, keypoints_f, data['contour_f'], ordered_landmark_f, img_s, sil_refined_s, keypoints_s, data['contour_s'], ordered_landmark_s, h_px_ratio, result)
        get_shoulder_type(img_f, sil_refined_f, keypoints_f, data['contour_f'], ordered_landmark_f, img_s, sil_refined_s, keypoints_s, data['contour_s'], ordered_landmark_s, h_px_ratio, result)
        get_arm_type(img_s, sil_refined_s, keypoints_s, h_px_ratio, result)
        get_back_style(img_s, sil_refined_s, keypoints_s,data['contour_s'], ordered_landmark_s, h_px_ratio, result)
        get_body_type(img_s, sil_refined_s, keypoints_s,data['contour_s'], ordered_landmark_s, h_px_ratio, result)
        get_belly_type(img_f, sil_refined_f, keypoints_f, data['contour_f'], ordered_landmark_f, img_s, sil_refined_s,
                       keypoints_s, data['contour_s'], ordered_landmark_s, h_px_ratio, result)

        return img_f, img_s, img_b, result
        # end_time = time.time()
...
if __name__ == '__main__':
    ...
    # # load pair info
    pair_path = '../body_test_data/pair.txt'
    IMG_DIR = '../body_test_data/test1/'
    OUT_DIR = '../body_test_data/result/'
    engine = body_type_measurement()

    pairs = load_pair_info(pair_text_file_path=pair_path)
    for img_name_f, img_name_b, img_name_s, height, weight in pairs:
        start_time = time.time()

        path_f = Path(f'{IMG_DIR}{img_name_f}')
        path_b = Path(f'{IMG_DIR}{img_name_b}')
        path_s = Path(f'{IMG_DIR}{img_name_s}')

        img_f = cv.imread(str(path_f))  # B,G,R order
        img_b = cv.imread(str(path_b))  # B,G,R order
        img_s = cv.imread(str(path_s))
        if img_f is None or img_s is None or img_b is None:
            raise ValueError('Image is not found')

        img_f, img_s, img_b, result = engine.handle_one_user_images(img_f=img_f, img_b=img_b, img_s=img_s,
                                                                    height=height)
        end_time = time.time()
        execution_time = end_time - start_time
        ...
        cv.imwrite(f'{OUT_DIR}{path_f.stem}_res.jpg', img_f)
        cv.imwrite(f'{OUT_DIR}{path_s.stem}_res.jpg', img_s)
        cv.imwrite(f'{OUT_DIR}{path_b.stem}_res.jpg', img_b)
